...
# gettext.bindtextdomain('uimge', "D:\\viu\\locale")
# gettext.textdomain('uimge')
...
class Main:
    ...
    def __init__(self):
        ...
        self.Outprint = {
                ...
                'usr_user-out':{
                    'out':'',
                    'help':'Set user output #url# - original image, #tmb# - preview image   Sample: [URL=%url%][IMG]%tmb%[/IMG][/URL]'},
                }


        self.key_hosts = '|'.join(['-'+i.split('_')[0] for i in self.Imagehosts.keys()])
        self.version = 'uimge-'+self.VERSION
        self.usage = 'python %%prog [%s] picture'%self.key_hosts
    ...
    def parseopt(self, argv):
        ...
        for host in self.Imagehosts.keys():
            ...
            group_1.add_option('-'+sp[0],'--'+sp[1],
                    ...
                    help='%s %s'%('Upload to',self.Imagehosts[host].host))
            ...
            if key != 'default':
                ...
                if key != 'usr_user-out':
                    group_3.add_option('--'+sp[0],'--'+sp[1],
                            ...
                            help=self.Outprint[key].get('help') or key)
                else:
        ...
        if self.opt.check == None:
            print 'No major option! Enter option [%s]...'%self.key_hosts
            parser.print_help()
            exit()
